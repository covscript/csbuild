...
while ifs.good()
    ...
    if !result.empty()
        ofs.println(result.str(1) + "Covariant Script Runtime - " + env.covscript_ver() + " (" + system.arch_name + ")" + result.str(2) + env.covscript_ver() + result.str(3))
        continue
    end
    result = reg2.match(str)
    if !result.empty()
        ofs.println(result.str(1) + system.arch_name + result.str(2))
        continue
    end
    ofs.println(str)
end